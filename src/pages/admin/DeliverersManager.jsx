  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold font-heading">Gestion des Agents ({deliverers.length})</h1>
        <Button className="flex items-center" onClick={() => {setEditingDeliverer(null); setIsModalOpen(true);}}>
          <Plus className="h-5 w-5 mr-2" />
          Savoir plus un Agent
        </Button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-6 border-b flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingDeliverer ? 'Modifier le agent' : 'Nouveau agent'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-secondary uppercase">Zone d'expertise</label>
                <Input name="zone" placeholder="ex: Abidjan (Cocody)" defaultValue={editingDeliverer?.zone} required />
              </div>
