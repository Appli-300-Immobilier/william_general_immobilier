  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold font-heading">Gestion des Biens ({products.length})</h1>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingProduct ? 'Modifier le bien' : 'Savoir plus un bien'}</h2>
              <button onClick={() => setIsModalOpen(false)}><X className="h-6 w-6" /></button>
            </div>
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input name="name" placeholder="Nom du bien" defaultValue={editingProduct?.name} required />
                <Input name="location" placeholder="Localisation" defaultValue={editingProduct?.location || "Abidjan (Cocody)"} required />
              </div>
              <Select 
                name="category" 
              ...
              />
              <div className="grid grid-cols-2 gap-4">
                <Input name="price" type="number" placeholder="Prix de vente" defaultValue={editingProduct?.price} required />
                <Input name="stock" type="number" placeholder="Disponibilité" defaultValue={editingProduct?.stock} required />
              </div>
              <textarea 
                name="description" 
